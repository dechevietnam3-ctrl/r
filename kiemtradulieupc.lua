...
local ExportWpBtn = Instance.new("TextButton", Page1); ExportWpBtn.Size = UDim2.new(0.18, 0, 0, 22); ExportWpBtn.Position = UDim2.new(0.60, 0, 0, 32); ExportWpBtn.Text = "📤 Export"; ExportWpBtn.BackgroundColor3 = Color3.fromRGB(150, 100, 0); ExportWpBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ExportWpBtn.Font = Enum.Font.SourceSansBold; ExportWpBtn.TextSize = 8; Instance.new("UICorner", ExportWpBtn).CornerRadius = UDim.new(0, 4)
local ImportWpBtn = Instance.new("TextButton", Page1); ImportWpBtn.Size = UDim2.new(0.18, 0, 0, 22); ImportWpBtn.Position = UDim2.new(0.79, 0, 0, 32); ImportWpBtn.Text = "📥 Import"; ImportWpBtn.BackgroundColor3 = Color3.fromRGB(40, 120, 80); ImportWpBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ImportWpBtn.Font = Enum.Font.SourceSansBold; ImportWpBtn.TextSize = 8; Instance.new("UICorner", ImportWpBtn).CornerRadius = UDim.new(0, 4)

local WpScroll = Instance.new("ScrollingFrame", Page1); WpScroll.Size = UDim2.new(1, 0, 1, -92); WpScroll.Position = UDim2.new(0, 0, 0, 60); WpScroll.BackgroundTransparency = 1; WpScroll.CanvasSize = UDim2.new(0, 0, 0, 0); WpScroll.ScrollBarThickness = 3
local WpListUI = Instance.new("UIListLayout", WpScroll); WpListUI.Padding = UDim.new(0, 4)
local SaveWpBtn = Instance.new("TextButton", Page1); SaveWpBtn.Size = UDim2.new(1, 0, 0, 24); SaveWpBtn.Position = UDim2.new(0, 0, 1, -24); SaveWpBtn.Text = "📌 Lưu Tọa Độ Hiện Tại Vào Danh Sách"; SaveWpBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 80); SaveWpBtn.TextColor3 = Color3.fromRGB(255, 255, 255); SaveWpBtn.Font = Enum.Font.SourceSansBold; SaveWpBtn.TextSize = 9; Instance.new("UICorner", SaveWpBtn).CornerRadius = UDim.new(0, 4)
    ...
    for _, model in pairs(Workspace:GetDescendants()) do
        if model:IsA("Model") and model:FindFirstChildOfClass("Humanoid") and not Players:GetPlayerFromCharacter(model) then
            ...
            if (filterText == "" or string.find(string.lower(model.Name), filterText)) and dist <= maxDistLimit then
                ...
                local TpBtn = Instance.new("TextButton", ItemFrame); TpBtn.Size = UDim2.new(0.24, 0, 0.7, 0); TpBtn.Position = UDim2.new(0.74, 0, 0.15, 0); TpBtn.Text = "Teleport"; TpBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 70); TpBtn.TextColor3 = Color3.fromRGB(255, 255, 255); TpBtn.Font = Enum.Font.SourceSansBold; TpBtn.TextSize = 9; Instance.new("UICorner", TpBtn).CornerRadius = UDim.new(0, 3)
...
local function updateBlockScanner()
    ...
    for _, obj in pairs(Workspace:GetChildren()) do
        if obj:IsA("BasePart") or obj:IsA("Model") then
            local ItemFrame = Instance.new("Frame", ScanScroll); ItemFrame.Size = UDim2.new(1, -5, 0, 26); ItemFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 25); Instance.new("UICorner", ItemFrame).CornerRadius = UDim.new(0, 4)
            ...
            local CopyPathBtn = Instance.new("TextButton", ItemFrame); CopyPathBtn.Size = UDim2.new(0.24, 0, 0.7, 0); CopyPathBtn.Position = UDim2.new(0.74, 0, 0.15, 0); CopyPathBtn.Text = "Copy Path"; CopyPathBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60); CopyPathBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyPathBtn.Font = Enum.Font.SourceSans; CopyPathBtn.TextSize = 9; Instance.new("UICorner", CopyPathBtn).CornerRadius = UDim.new(0, 3)
            CopyPathBtn.MouseButton1Click:Connect(function() if setclipboard then setclipboard(obj:GetFullName()) end end)
        end
    end
...
local function buildStatDetails(plr, parentFrame)
    ...
    for _, stat in pairs(leaderstats:GetChildren()) do
        local StatFrame = Instance.new("Frame", parentFrame); StatFrame.Size = UDim2.new(1, -4, 0, 22); StatFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 22); Instance.new("UICorner", StatFrame).CornerRadius = UDim.new(0, 4)
        ...
        local CopyValBtn = Instance.new("TextButton", StatFrame); CopyValBtn.Size = UDim2.new(0.36, 0, 0.7, 0); CopyValBtn.Position = UDim2.new(0.61, 0, 0.15, 0); CopyValBtn.Text = "📋 Copy Value"; CopyValBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 50); CopyValBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyValBtn.Font = Enum.Font.SourceSansBold; CopyValBtn.TextSize = 8; Instance.new("UICorner", CopyValBtn).CornerRadius = UDim.new(0, 3)
...
local function updateStatsInspector()
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        ...
        local CardFrame = Instance.new("Frame", StatsScroll); CardFrame.Size = UDim2.new(1, -5, 0, 125); CardFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 25); Instance.new("UICorner", CardFrame).CornerRadius = UDim.new(0, 5)
        ...
        local StatsContainer = Instance.new("ScrollingFrame", CardFrame); StatsContainer.Size = UDim2.new(0.96, 0, 0, 98); StatsContainer.Position = UDim2.new(0.02, 0, 0.18, 0); StatsContainer.BackgroundColor3 = Color3.fromRGB(12, 12, 15); StatsContainer.CanvasSize = UDim2.new(0, 0, 0, 0); StatsContainer.ScrollBarThickness = 3
