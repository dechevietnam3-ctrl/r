--=========================================================  
-- ULTIMATE 9-TAB CUSTOM TOOLKIT v12.1 (NÂNG CẤP TOÀN DIỆN)  
        ...
        if AntiBanActive then
            pcall(function()
                ...
                if char then
                    ...
                    -- Ở đây có thể thêm các biện pháp chống phát hiện
                    -- Ví dụ: reset WalkSpeed về giá trị hợp lệ nếu bị thay đổi đột ngột
                end
            end)
        end
    end
...
ScreenGui.Name = "CustomInspectorUI_v12_1"  
...
MainFrame.Size = UDim2.new(0, 680, 0, 520) -- Tăng chiều cao để chứa thêm nội dung  
...
TitleLabel.Text = "🚀 ULTIMATE TOOLKIT v12.1 (NÂNG CẤP TOÀN DIỆN)"  
...
local SaveDataBtn = Instance.new("TextButton", Page1)  
SaveDataBtn.Size = UDim2.new(0.14, 0, 0, 22)  
SaveDataBtn.Position = UDim2.new(0.77, 0, 0, 32)  
SaveDataBtn.Text = "💾 Save Data"  
SaveDataBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 120)  
SaveDataBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
SaveDataBtn.Font = Enum.Font.SourceSansBold  
SaveDataBtn.TextSize = 8  
Instance.new("UICorner", SaveDataBtn).CornerRadius = UDim.new(0, 4)  

local AbnormalToggle = Instance.new("TextButton", Page1)  
AbnormalToggle.Size = UDim2.new(0.22, 0, 0, 22)  
AbnormalToggle.Position = UDim2.new(0.78, 0, 0, 58)  -- Đẩy xuống  
...
AbnormalScroll.Position = UDim2.new(0, 0, 0, 84)  
...
WpScroll.Position = UDim2.new(0, 0, 0, 168)  
...
-- ================== KHU VỰC ĐẶC BIỆT ==================  
local QuickZoneFrame = Instance.new("Frame", Page1)  
QuickZoneFrame.Size = UDim2.new(1, 0, 0, 80)  
QuickZoneFrame.Position = UDim2.new(0, 0, 0, 58)  
QuickZoneFrame.BackgroundColor3 = Color3.fromRGB(12, 12, 16)  
Instance.new("UICorner", QuickZoneFrame).CornerRadius = UDim.new(0, 4)  

local QLabel = Instance.new("TextLabel", QuickZoneFrame)  
QLabel.Size = UDim2.new(1, 0, 0, 18)  
QLabel.Text = "📍 KHU VỰC NHANH (Spawn, Hub, Shop...)"  
QLabel.TextColor3 = Color3.fromRGB(0, 255, 200)  
QLabel.BackgroundTransparency = 1  
QLabel.Font = Enum.Font.SourceSansBold  
QLabel.TextSize = 10  
QLabel.TextXAlignment = Enum.TextXAlignment.Left  

local QScroll = Instance.new("ScrollingFrame", QuickZoneFrame)  
QScroll.Size = UDim2.new(1, 0, 1, -20)  
QScroll.Position = UDim2.new(0, 0, 0, 20)  
QScroll.BackgroundTransparency = 1  
QScroll.CanvasSize = UDim2.new(0, 0, 0, 0)  
QScroll.ScrollBarThickness = 3  

local QListUI = Instance.new("UIListLayout", QScroll)  
QListUI.Padding = UDim.new(0, 2)  

local function scanZones()  
    for _, child in pairs(QScroll:GetChildren()) do if child:IsA("Frame") then child:Destroy() end end  
    local keywords = {"spawn", "hub", "lobby", "shop", "start", "teleport", "portal", "zone", "base"}  
    for _, part in pairs(Workspace:GetDescendants()) do  
        if part:IsA("BasePart") and part.Name then  
            local nameLow = string.lower(part.Name)  
            for _, kw in ipairs(keywords) do  
                if string.find(nameLow, kw) then  
                    local Frame = Instance.new("Frame", QScroll)  
                    ...
                    Label.Text = string.format("📍 %s (%.1f, %.1f, %.1f)", part.Name, part.Position.X, part.Position.Y, part.Position.Z)  
                    ...
                    TpBtn.Position = UDim2.new(0.63, 0, 0.1, 0)  
                    TpBtn.Text = "Teleport"  
                    ...
                    TpBtn.MouseButton1Click:Connect(function()  
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then  
                            LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(part.Position + Vector3.new(0, 2, 0))  
                        end  
                    end)  
                    break  
                end  
    ...
    QScroll.CanvasSize = UDim2.new(0, 0, 0, QListUI.AbsoluteContentSize.Y)  
end  

task.spawn(function()  
    while task.wait(3) do  
        if Page1.Visible then scanZones() end  
    end  
end)  
scanZones()  
...
--=========================================================  
-- TAB 2: NPC RADAR + SCRIPT TRONG NPC + ĐIỀU KHIỂN HƯỚNG  
--=========================================================  
...
-- Điều khiển NPC  
local ControlNpcBtn = Instance.new("TextButton", Page2)  
ControlNpcBtn.Size = UDim2.new(0.3, 0, 0, 24)  
ControlNpcBtn.Position = UDim2.new(0, 0, 0, 60)  
ControlNpcBtn.Text = "🎯 Điều khiển NPC (Chọn mục tiêu)"  
ControlNpcBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180)  
ControlNpcBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ControlNpcBtn.Font = Enum.Font.SourceSansBold  
ControlNpcBtn.TextSize = 9  
Instance.new("UICorner", ControlNpcBtn).CornerRadius = UDim.new(0, 4)  

local NpcTargetBox = Instance.new("TextBox", Page2)  
NpcTargetBox.Size = UDim2.new(0.3, 0, 0, 24)  
NpcTargetBox.Position = UDim2.new(0.35, 0, 0, 60)  
NpcTargetBox.PlaceholderText = "Tên NPC cần điều khiển..."  
NpcTargetBox.Text = ""  
NpcTargetBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28)  
NpcTargetBox.TextColor3 = Color3.fromRGB(255, 255, 255)  
NpcTargetBox.Font = Enum.Font.SourceSans  
NpcTargetBox.TextSize = 9  
Instance.new("UICorner", NpcTargetBox).CornerRadius = UDim.new(0, 4)  

local ControlDirBtn = Instance.new("TextButton", Page2)  
ControlDirBtn.Size = UDim2.new(0.3, 0, 0, 24)  
ControlDirBtn.Position = UDim2.new(0.68, 0, 0, 60)  
ControlDirBtn.Text = "🔄 Xoay hướng NPC"  
ControlDirBtn.BackgroundColor3 = Color3.fromRGB(180, 90, 0)  
ControlDirBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ControlDirBtn.Font = Enum.Font.SourceSansBold  
ControlDirBtn.TextSize = 9  
Instance.new("UICorner", ControlDirBtn).CornerRadius = UDim.new(0, 4)  

ControlNpcBtn.MouseButton1Click:Connect(function()  
    local targetName = string.lower(NpcTargetBox.Text)  
    ...
    for _, model in pairs(Workspace:GetDescendants()) do  
        if model:IsA("Model") and model:FindFirstChildOfClass("Humanoid") and not Players:GetPlayerFromCharacter(model) then  
            if string.find(string.lower(model.Name), targetName) then  
...
ControlDirBtn.MouseButton1Click:Connect(function()  
    local targetName = string.lower(NpcTargetBox.Text)  
    if targetName == "" then return end  
    for _, model in pairs(Workspace:GetDescendants()) do  
        if model:IsA("Model") and model:FindFirstChildOfClass("Humanoid") and not Players:GetPlayerFromCharacter(model) then  
...
NpcScroll.Size = UDim2.new(1, 0, 1, -90)  
NpcScroll.Position = UDim2.new(0, 0, 0, 90)  
...
--=========================================================  
-- TAB 3: BLOCKS + SCRIPT, ESP, TP, TỌA ĐỘ + KHOẢNG CÁCH  
--=========================================================  
...
local function updateBlockScanner()  
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do  
        if obj:IsA("BasePart") then  
            ...
            local dist = myHrp and (myHrp.Position - obj.Position).Magnitude or 0  
            ...
            local InfoText = Instance.new("TextLabel", ItemFrame); InfoText.Size = UDim2.new(0.5, 0, 0.5, 0); InfoText.Position = UDim2.new(0.02, 0, 0, 0); InfoText.Text = string.format("📦 %s [%s] | 📏 %.1fm\n(%.1f, %.1f, %.1f)", obj.Name, obj.ClassName, dist, obj.Position.X, obj.Position.Y, obj.Position.Z); InfoText.TextColor3 = Color3.fromRGB(200, 200, 200); InfoText.BackgroundTransparency = 1; InfoText.Font = Enum.Font.SourceSans; InfoText.TextSize = 9; InfoText.TextXAlignment = Enum.TextXAlignment.Left  
...
--=========================================================  
-- TAB 4: BACKPACK + VẬT PHẨM TRONG MAP  
--=========================================================  
-- Vật phẩm trong map  
local ItemScroll = Instance.new("ScrollingFrame", Page4)  
ItemScroll.Size = UDim2.new(1, 0, 0, 120)  
ItemScroll.Position = UDim2.new(0, 0, 0, 0)  
ItemScroll.BackgroundColor3 = Color3.fromRGB(10, 10, 15)  
ItemScroll.CanvasSize = UDim2.new(0, 0, 0, 0)  
ItemScroll.ScrollBarThickness = 3  
Instance.new("UICorner", ItemScroll).CornerRadius = UDim.new(0, 4)  

local ItemListUI = Instance.new("UIListLayout", ItemScroll)  
ItemListUI.Padding = UDim.new(0, 2)  

local function scanMapItems()  
    for _, child in pairs(ItemScroll:GetChildren()) do if child:IsA("Frame") then child:Destroy() end end  
    local myHrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")  
    for _, obj in pairs(Workspace:GetDescendants()) do  
        if obj:IsA("Tool") or (obj:IsA("Model") and obj:FindFirstChildOfClass("Tool")) then  
            ...
            if tool then  
                local pos = tool:IsA("BasePart") and tool.Position or (tool.PrimaryPart and tool.PrimaryPart.Position) or Vector3.new(0,0,0)  
                local dist = myHrp and (myHrp.Position - pos).Magnitude or 0  
                local frame = Instance.new("Frame", ItemScroll)  
                frame.Size = UDim2.new(1, -5, 0, 20)  
                frame.BackgroundColor3 = Color3.fromRGB(22, 22, 28)  
                Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 3)  
                local label = Instance.new("TextLabel", frame)  
                ...
                label.Position = UDim2.new(0.02, 0, 0, 0)  
                label.Text = string.format("🔧 %s (%.1fm)", tool.Name, dist)  
                ...
                label.TextXAlignment = Enum.TextXAlignment.Left  
                local TpBtn = Instance.new("TextButton", frame)  
                TpBtn.Size = UDim2.new(0.45, 0, 0.8, 0)  
                TpBtn.Position = UDim2.new(0.53, 0, 0.1, 0)  
                TpBtn.Text = "Teleport & Lấy"  
                TpBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 70)  
                TpBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
                TpBtn.Font = Enum.Font.SourceSansBold  
                TpBtn.TextSize = 8  
    ...
    ItemScroll.CanvasSize = UDim2.new(0, 0, 0, ItemListUI.AbsoluteContentSize.Y)  
end  

task.spawn(function()  
    while task.wait(3) do  
        if Page4.Visible then scanMapItems() end  
    end  
end)  

-- Backpack inspector  
local EquipAllBtn = Instance.new("TextButton", Page4); EquipAllBtn.Size = UDim2.new(0.48, 0, 0, 22); EquipAllBtn.Position = UDim2.new(0, 0, 0, 126); EquipAllBtn.Text = "⚔️ Equip All Tools"; EquipAllBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); EquipAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255); EquipAllBtn.Font = Enum.Font.SourceSansBold; EquipAllBtn.TextSize = 9; Instance.new("UICorner", EquipAllBtn).CornerRadius = UDim.new(0, 4)  
local DropAllBtn = Instance.new("TextButton", Page4); DropAllBtn.Size = UDim2.new(0.48, 0, 0, 22); DropAllBtn.Position = UDim2.new(0.51, 0, 0, 126); DropAllBtn.Text = "🗑️ Drop Holding Tool"; DropAllBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50); DropAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255); DropAllBtn.Font = Enum.Font.SourceSansBold; DropAllBtn.TextSize = 9; Instance.new("UICorner", DropAllBtn).CornerRadius = UDim.new(0, 4)  

local InvScroll = Instance.new("ScrollingFrame", Page4); InvScroll.Size = UDim2.new(1, 0, 1, -154); InvScroll.Position = UDim2.new(0, 0, 0, 154); InvScroll.BackgroundTransparency = 1; InvScroll.CanvasSize = UDim2.new(0, 0, 0, 0); InvScroll.ScrollBarThickness = 3  
...
--=========================================================  
-- TAB 5: STATS INSPECTOR + TÌM SCRIPT TĂNG CHỈ SỐ + KIỂM TRA HOẠT ĐỘNG  
--=========================================================  
...
local function buildStatDetails(plr, parentFrame)  
    ...
    for _, stat in pairs(leaderstats:GetChildren()) do  
        ...
        FindScriptBtn.MouseButton1Click:Connect(function()  
            ...
            local popup = Instance.new("Frame", ScreenGui)  
            popup.Size = UDim2.new(0, 550, 0, 350)  
            popup.Position = UDim2.new(0.5, -275, 0.5, -175)  
            popup.BackgroundColor3 = Color3.fromRGB(20, 20, 30)  
            Instance.new("UICorner", popup).CornerRadius = UDim.new(0, 8)  
            local close = Instance.new("TextButton", popup); close.Size = UDim2.new(0, 30, 0, 30); close.Position = UDim2.new(1, -35, 0, 5); close.Text = "✕"; close.BackgroundColor3 = Color3.fromRGB(50, 50, 50); close.TextColor3 = Color3.fromRGB(255, 255, 255); close.Font = Enum.Font.SourceSansBold; close.TextSize = 12; close.MouseButton1Click:Connect(function() popup:Destroy() end)  
                ...
                for _, scr in ipairs(scripts) do  
                    ...
                    CheckBtn.MouseButton1Click:Connect(function()  
                        ...
                        if scr:IsA("LocalScript") or scr:IsA("Script") then  
                            isRunning = true  -- không có cách đơn giản, đặt true  
                        end  
                        CheckBtn.Text = isRunning and "✅ Đang chạy" or "❌ Không chạy"  
                        CheckBtn.BackgroundColor3 = isRunning and Color3.fromRGB(0, 150, 80) or Color3.fromRGB(150, 0, 0)  
                    end)  
                    local copy = Instance.new("TextButton", frame); copy.Size = UDim2.new(0.4, 0, 0.8, 0); copy.Position = UDim2.new(0.56, 0, 0.1, 0); copy.Text = "📋 Copy Code"; copy.BackgroundColor3 = Color3.fromRGB(0, 120, 180); copy.TextColor3 = Color3.fromRGB(255, 255, 255); copy.Font = Enum.Font.SourceSansBold; copy.TextSize = 8; Instance.new("UICorner", copy).CornerRadius = UDim.new(0, 3)  
                    copy.MouseButton1Click:Connect(function() if setclipboard then setclipboard(getScriptCode(scr)) end end)  
                end  
            end  
            scroll.CanvasSize = UDim2.new(0, 0, 0, listUI.AbsoluteContentSize.Y)  
...
-- Nâng cấp Spy  
local BlockRemoteBtn = Instance.new("TextButton", Page6)  
BlockRemoteBtn.Size = UDim2.new(0.18, 0, 0, 24)  
BlockRemoteBtn.Position = UDim2.new(0, 0, 0, 60)  
BlockRemoteBtn.Text = "🚫 Block Remote"  
BlockRemoteBtn.BackgroundColor3 = Color3.fromRGB(180, 40, 40)  
BlockRemoteBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
BlockRemoteBtn.Font = Enum.Font.SourceSansBold  
BlockRemoteBtn.TextSize = 9  
Instance.new("UICorner", BlockRemoteBtn).CornerRadius = UDim.new(0, 4)  

local ReplayRemoteBtn = Instance.new("TextButton", Page6)  
ReplayRemoteBtn.Size = UDim2.new(0.18, 0, 0, 24)  
ReplayRemoteBtn.Position = UDim2.new(0.2, 0, 0, 60)  
ReplayRemoteBtn.Text = "🔁 Replay Last"  
ReplayRemoteBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180)  
ReplayRemoteBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ReplayRemoteBtn.Font = Enum.Font.SourceSansBold  
ReplayRemoteBtn.TextSize = 9  
Instance.new("UICorner", ReplayRemoteBtn).CornerRadius = UDim.new(0, 4)  

local ExportSpyBtn = Instance.new("TextButton", Page6)  
ExportSpyBtn.Size = UDim2.new(0.18, 0, 0, 24)  
ExportSpyBtn.Position = UDim2.new(0.4, 0, 0, 60)  
ExportSpyBtn.Text = "📤 Export Log"  
ExportSpyBtn.BackgroundColor3 = Color3.fromRGB(150, 100, 0)  
ExportSpyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ExportSpyBtn.Font = Enum.Font.SourceSansBold  
ExportSpyBtn.TextSize = 9  
Instance.new("UICorner", ExportSpyBtn).CornerRadius = UDim.new(0, 4)  

local SpyScroll = Instance.new("ScrollingFrame", Page6); SpyScroll.Size = UDim2.new(1, 0, 1, -90); SpyScroll.Position = UDim2.new(0, 0, 0, 90); SpyScroll.BackgroundTransparency = 1; SpyScroll.CanvasSize = UDim2.new(0, 0, 0, 0); SpyScroll.ScrollBarThickness = 3  
...
local lastRemote = nil; local lastArgs = {}  
...
BlockRemoteBtn.MouseButton1Click:Connect(function()  
    if lastRemote then  
        blacklistedRemoteNames[lastRemote.Name] = true  
    end  
end)  

ReplayRemoteBtn.MouseButton1Click:Connect(function()  
    if lastRemote then  
        pcall(function()  
            if lastRemote:IsA("RemoteEvent") then lastRemote:FireServer(unpack(lastArgs))  
            elseif lastRemote:IsA("RemoteFunction") then lastRemote:InvokeServer(unpack(lastArgs)) end  
        end)  
    end  
end)  

ExportSpyBtn.MouseButton1Click:Connect(function()  
    local text = ""  
    for _, frame in pairs(SpyScroll:GetChildren()) do  
        if frame:IsA("Frame") then  
            local info = frame:FindFirstChild("InfoText")  
            if info then text = text .. info.Text .. "\n" end  
        end  
    end  
...
local function logRemoteEvent(remote, args)  
    ...
    lastRemote = remote; lastArgs = args  
...
local ExportLogBtn = Instance.new("TextButton", Page7)  
ExportLogBtn.Size = UDim2.new(0.12, 0, 0, 22)  
ExportLogBtn.Position = UDim2.new(0.86, 0, 0, 0)  
ExportLogBtn.Text = "📤 Export"  
ExportLogBtn.BackgroundColor3 = Color3.fromRGB(150, 100, 0)  
ExportLogBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ExportLogBtn.Font = Enum.Font.SourceSansBold  
ExportLogBtn.TextSize = 9  
Instance.new("UICorner", ExportLogBtn).CornerRadius = UDim.new(0, 3)  
...
ExportLogBtn.MouseButton1Click:Connect(function()  
    local allText = ""  
    for _, log in ipairs(logHistory) do  
        allText = allText .. log.msg .. "\n"  
    end  
    if setclipboard then setclipboard(allText) end  
end)  

--=========================================================  
-- TAB 8: GUI INSPECTOR + TẠO NÚT CUSTOM  
--=========================================================  
...
-- Tạo nút custom  
local CustomFrame = Instance.new("Frame", Page8)  
CustomFrame.Size = UDim2.new(1, 0, 0, 90)  
CustomFrame.Position = UDim2.new(0, 0, 0, 30)  
CustomFrame.BackgroundColor3 = Color3.fromRGB(12, 12, 16)  
Instance.new("UICorner", CustomFrame).CornerRadius = UDim.new(0, 4)  

local CmdLabel = Instance.new("TextLabel", CustomFrame)  
CmdLabel.Size = UDim2.new(1, 0, 0, 18)  
CmdLabel.Text = "✏️ Tạo nút với lệnh Lua"  
CmdLabel.TextColor3 = Color3.fromRGB(0, 255, 200)  
CmdLabel.BackgroundTransparency = 1  
CmdLabel.Font = Enum.Font.SourceSansBold  
CmdLabel.TextSize = 10  

local CmdBox = Instance.new("TextBox", CustomFrame)  
CmdBox.Size = UDim2.new(0.7, 0, 0, 24)  
CmdBox.Position = UDim2.new(0.02, 0, 0, 20)  
CmdBox.PlaceholderText = "Nhập lệnh (ví dụ: print('hello'))..."  
CmdBox.Text = ""  
CmdBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28)  
...
local CreateBtn = Instance.new("TextButton", CustomFrame)  
CreateBtn.Size = UDim2.new(0.25, 0, 0, 24)  
CreateBtn.Position = UDim2.new(0.73, 0, 0, 20)  
CreateBtn.Text = "➕ Tạo Nút"  
CreateBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 80)  
...
CreateBtn.MouseButton1Click:Connect(function()  
    ...
    local newBtn = Instance.new("TextButton", ScreenGui)  
    newBtn.Size = UDim2.new(0, 120, 0, 30)  
    newBtn.Position = UDim2.new(math.random(0, 80)/100, 0, math.random(0, 80)/100, 0)  
    newBtn.Text = "Custom"  
    newBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 40)  
    newBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
    newBtn.Font = Enum.Font.SourceSansBold  
    newBtn.TextSize = 10  
    Instance.new("UICorner", newBtn).CornerRadius = UDim.new(0, 4)  
    newBtn.Draggable = true  
    newBtn.Active = true  
    newBtn.MouseButton1Click:Connect(function()  
        pcall(function() loadstring(cmd)() end)  
    end)  
    table.insert(customButtons, newBtn)  
    local delBtn = Instance.new("TextButton", newBtn)  
    ...
    delBtn.MouseButton1Click:Connect(function()  
        newBtn:Destroy()  
        for i, v in pairs(customButtons) do if v == newBtn then table.remove(customButtons, i) break end end  
    end)  
end)  

local function createDefaultButtons()  
    local defaults = {  
        {"🦘 Nhảy", "game:GetService('Players').LocalPlayer.Character.Humanoid.Jump = true"},  
        {"🏃 Chạy nhanh", "game:GetService('Players').LocalPlayer.Character.Humanoid.WalkSpeed = 50"},  
        {"🪂 Bay", "game:GetService('Players').LocalPlayer.Character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)"},  
        {"📦 NoClip", "game:GetService('Players').LocalPlayer.Character.HumanoidRootPart.CanCollide = false"},  
        {"📡 Teleport về spawn", "game:GetService('Players').LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(0, 10, 0)"}  
    }  
    for _, data in ipairs(defaults) do  
        local btn = Instance.new("TextButton", ScreenGui)  
        btn.Size = UDim2.new(0, 100, 0, 25)  
        btn.Position = UDim2.new(math.random(0, 80)/100, 0, math.random(0, 80)/100, 0)  
        btn.Text = data[1]  
        ...
        btn.Active = true  
        btn.MouseButton1Click:Connect(function()  
            pcall(function() loadstring(data[2])() end)  
        end)  
        table.insert(customButtons, btn)  
    end  
end  
createDefaultButtons()  

local GuiScroll = Instance.new("ScrollingFrame", Page8); GuiScroll.Size = UDim2.new(1, 0, 1, -130); GuiScroll.Position = UDim2.new(0, 0, 0, 130); GuiScroll.BackgroundTransparency = 1; GuiScroll.CanvasSize = UDim2.new(0, 0, 0, 0); GuiScroll.ScrollBarThickness = 3  
...
--=========================================================  
-- TAB 9: HITBOX MASTER (GIỮ NGUYÊN, CÓ THỂ NÂNG CẤP THÊM)  
--=========================================================  
...
local function saveData()  
    ...
    if writefile then  
        writefile("toolkit_save.json", json)  
...
local function loadData()  
    if readfile then  
        local content = readfile("toolkit_save.json")  
...
task.spawn(function() loadData() end)  
game:BindToClose(saveData)  
SaveDataBtn.MouseButton1Click:Connect(saveData)  
