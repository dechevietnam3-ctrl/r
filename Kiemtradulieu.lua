--=========================================================  
-- ULTIMATE 9-TAB CUSTOM TOOLKIT v13.0 (NÂNG CẤP THEO YÊU CẦU)  
...
ScreenGui.Name = "CustomInspectorUI_v13_0"  
...
TitleLabel.Text = "🚀 ULTIMATE TOOLKIT v13.0 (NÂNG CẤP THEO YÊU CẦU)"  
...
--=========================================================  
-- TAB 1: TỌA ĐỘ & WAYPOINTS + PHÁT HIỆN DỊCH CHUYỂN BẤT THƯỜNG (nâng cấp: ghi cả điểm đến và điểm bắt đầu)  
--=========================================================  
...
-- Khung hiển thị danh sách dịch chuyển bất thường (nâng cấp: hiển thị cả điểm bắt đầu và kết thúc)  
...
-- Nút Clear Abnormal Log  
local ClearAbnormalBtn = Instance.new("TextButton", Page1)  
ClearAbnormalBtn.Size = UDim2.new(0.15, 0, 0, 22)  
ClearAbnormalBtn.Position = UDim2.new(0, 0, 0, 58)  
ClearAbnormalBtn.Text = "🗑️ Clear Log"  
ClearAbnormalBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)  
ClearAbnormalBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ClearAbnormalBtn.Font = Enum.Font.SourceSansBold  
ClearAbnormalBtn.TextSize = 8  
Instance.new("UICorner", ClearAbnormalBtn).CornerRadius = UDim.new(0, 4)  
ClearAbnormalBtn.MouseButton1Click:Connect(function()  
	abnormalList = {}  
	for _, child in pairs(AbnormalScroll:GetChildren()) do if child:IsA("Frame") then child:Destroy() end end  
	AbnormalScroll.CanvasSize = UDim2.new(0, 0, 0, 0)  
end)  

local function addAbnormalLog(fromPos, toPos, cframe)  
	local time = os.date("%H:%M:%S")  
	table.insert(abnormalList, {from = fromPos, to = toPos, cframe = cframe, time = time})  
	-- Cập nhật giao diện  
	for _, child in pairs(AbnormalScroll:GetChildren()) do if child:IsA("Frame") then child:Destroy() end end  
	for i, data in ipairs(abnormalList) do  
		...
		local Label = Instance.new("TextLabel", Frame); Label.Size = UDim2.new(0.7, 0, 1, 0); Label.Position = UDim2.new(0.02, 0, 0, 0);  
		Label.Text = string.format("[%s] From (%.1f,%.1f,%.1f) -> To (%.1f,%.1f,%.1f)", data.time, data.from.X, data.from.Y, data.from.Z, data.to.X, data.to.Y, data.to.Z)  
		Label.TextColor3 = Color3.fromRGB(255, 200, 100); Label.BackgroundTransparency = 1; Label.Font = Enum.Font.SourceSans; Label.TextSize = 9; Label.TextXAlignment = Enum.TextXAlignment.Left  
		...
		if char and char:FindFirstChild("HumanoidRootPart") then  
			...
			if lastPos ~= Vector3.new(0,0,0) then  
				...
				if dist > 100 then -- ngưỡng bất thường  
					addAbnormalLog(lastPos, curPos, hrp.CFrame)  
				end  
			end  
			lastPos = curPos  
		end  
...
--=========================================================  
-- TAB 2: NPC RADAR + SCRIPT TRONG NPC + THEO DÕI NPC (nâng cấp: thêm theo dõi)  
--=========================================================  
local NpcSearchBox = Instance.new("TextBox", Page2); NpcSearchBox.Size = UDim2.new(0.3, 0, 0, 24); NpcSearchBox.PlaceholderText = "🔍 Tên NPC..."; NpcSearchBox.Text = ""; NpcSearchBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28); NpcSearchBox.TextColor3 = Color3.fromRGB(255, 255, 255); NpcSearchBox.Font = Enum.Font.SourceSans; NpcSearchBox.TextSize = 10; Instance.new("UICorner", NpcSearchBox).CornerRadius = UDim.new(0, 4)  
local MaxDistBox = Instance.new("TextBox", Page2); MaxDistBox.Size = UDim2.new(0.14, 0, 0, 24); MaxDistBox.Position = UDim2.new(0.32, 0, 0, 0); MaxDistBox.PlaceholderText = "Max Dist..."; MaxDistBox.Text = ""; MaxDistBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28); MaxDistBox.TextColor3 = Color3.fromRGB(255, 255, 255); MaxDistBox.Font = Enum.Font.SourceSans; MaxDistBox.TextSize = 9; Instance.new("UICorner", MaxDistBox).CornerRadius = UDim.new(0, 4)  
local EspToggleBtn = Instance.new("TextButton", Page2); EspToggleBtn.Size = UDim2.new(0.16, 0, 0, 24); EspToggleBtn.Position = UDim2.new(0.48, 0, 0, 0); EspToggleBtn.Text = "👁️ ESP: OFF"; EspToggleBtn.BackgroundColor3 = Color3.fromRGB(140, 35, 35); EspToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255); EspToggleBtn.Font = Enum.Font.SourceSansBold; EspToggleBtn.TextSize = 9; Instance.new("UICorner", EspToggleBtn).CornerRadius = UDim.new(0, 4)  
local ShowScriptsToggle = Instance.new("TextButton", Page2); ShowScriptsToggle.Size = UDim2.new(0.16, 0, 0, 24); ShowScriptsToggle.Position = UDim2.new(0.66, 0, 0, 0); ShowScriptsToggle.Text = "📜 Scripts: OFF"; ShowScriptsToggle.BackgroundColor3 = Color3.fromRGB(50, 50, 50); ShowScriptsToggle.TextColor3 = Color3.fromRGB(255, 255, 255); ShowScriptsToggle.Font = Enum.Font.SourceSansBold; ShowScriptsToggle.TextSize = 8; Instance.new("UICorner", ShowScriptsToggle).CornerRadius = UDim.new(0, 4)  
-- Nút theo dõi NPC (tự động cập nhật vị trí)  
local TrackNpcToggle = Instance.new("TextButton", Page2); TrackNpcToggle.Size = UDim2.new(0.16, 0, 0, 24); TrackNpcToggle.Position = UDim2.new(0.84, 0, 0, 0); TrackNpcToggle.Text = "🎯 Track: OFF"; TrackNpcToggle.BackgroundColor3 = Color3.fromRGB(50, 50, 50); TrackNpcToggle.TextColor3 = Color3.fromRGB(255, 255, 255); TrackNpcToggle.Font = Enum.Font.SourceSansBold; TrackNpcToggle.TextSize = 8; Instance.new("UICorner", TrackNpcToggle).CornerRadius = UDim.new(0, 4)  
...
TrackNpcToggle.MouseButton1Click:Connect(function()  
	trackActive = not trackActive  
	TrackNpcToggle.Text = trackActive and "🎯 Track: ON" or "🎯 Track: OFF"  
	TrackNpcToggle.BackgroundColor3 = trackActive and Color3.fromRGB(0, 140, 70) or Color3.fromRGB(50, 50, 50)  
	if not trackActive then trackedNpc = nil end  
end)  
	...
	for _, model in pairs(Workspace:GetDescendants()) do  
		if model:IsA("Model") and model:FindFirstChildOfClass("Humanoid") and not Players:GetPlayerFromCharacter(model) then  
			...
			if (filterText == "" or string.find(string.lower(model.Name), filterText)) and dist <= maxDistLimit then  
				...
				local InfoText = Instance.new("TextLabel", ItemFrame); InfoText.Size = UDim2.new(0.55, 0, 1, 0); InfoText.Position = UDim2.new(0.02, 0, 0, 0);  
				if trackActive and trackedNpc == model then  
					InfoText.Text = string.format("🎯 %s | HP: %d/%d | (%dm) [Tracking]", model.Name, math.floor(hum.Health), math.floor(hum.MaxHealth), dist)  
				else  
					InfoText.Text = string.format("👾 %s | HP: %d/%d | (%dm)", model.Name, math.floor(hum.Health), math.floor(hum.MaxHealth), dist)  
				end  
				InfoText.TextColor3 = Color3.fromRGB(220, 220, 220); InfoText.BackgroundTransparency = 1; InfoText.Font = Enum.Font.SourceSans; InfoText.TextSize = 10; InfoText.TextXAlignment = Enum.TextXAlignment.Left  
				local TpBtn = Instance.new("TextButton", ItemFrame); TpBtn.Size = UDim2.new(0.16, 0, 0.7, 0); TpBtn.Position = UDim2.new(0.58, 0, 0.15, 0); TpBtn.Text = "Teleport"; TpBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 70); TpBtn.TextColor3 = Color3.fromRGB(255, 255, 255); TpBtn.Font = Enum.Font.SourceSansBold; TpBtn.TextSize = 9; Instance.new("UICorner", TpBtn).CornerRadius = UDim.new(0, 3)  
				TpBtn.MouseButton1Click:Connect(function() if myHrp and hrp then myHrp.CFrame = hrp.CFrame * CFrame.new(0, 2, 3) end end)  
				-- Nút Track  
				local TrackBtn = Instance.new("TextButton", ItemFrame); TrackBtn.Size = UDim2.new(0.16, 0, 0.7, 0); TrackBtn.Position = UDim2.new(0.76, 0, 0.15, 0); TrackBtn.Text = "🎯 Track"; TrackBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 150); TrackBtn.TextColor3 = Color3.fromRGB(255, 255, 255); TrackBtn.Font = Enum.Font.SourceSansBold; TrackBtn.TextSize = 8; Instance.new("UICorner", TrackBtn).CornerRadius = UDim.new(0, 3)  
				TrackBtn.MouseButton1Click:Connect(function()  
					if not trackActive then trackActive = true; TrackNpcToggle.Text = "🎯 Track: ON"; TrackNpcToggle.BackgroundColor3 = Color3.fromRGB(0, 140, 70) end  
					trackedNpc = (trackedNpc == model) and nil or model  
					updateNpcList()  
				end)  
...
-- Vòng lặp cập nhật tracking  
task.spawn(function()  
	while task.wait(1) do  
		if trackActive and trackedNpc then  
			pcall(function()  
				local myHrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")  
				local hrp = trackedNpc:FindFirstChild("HumanoidRootPart") or trackedNpc.PrimaryPart  
				if myHrp and hrp then  
					local dist = (hrp.Position - myHrp.Position).Magnitude  
					-- Cập nhật label trong danh sách  
					for _, frame in pairs(NpcScroll:GetChildren()) do  
						if frame:IsA("Frame") then  
							local label = frame:FindFirstChildOfClass("TextLabel")  
							if label and label.Text:find(trackedNpc.Name) then  
								local newText = string.gsub(label.Text, "(%d+m)", math.floor(dist).."m")  
								if not label.Text:find("Tracking") then  
									newText = newText .. " [Tracking]"  
								end  
								label.Text = newText  
							end  
						end  
...
--=========================================================  
-- TAB 3: BLOCKS + SCRIPT, ESP, TP, TỌA ĐỘ + BỘ LỌC (nâng cấp: thêm filter)  
--=========================================================  
local InstantHoldBtn = Instance.new("TextButton", Page3); InstantHoldBtn.Size = UDim2.new(0.16, 0, 0, 24); InstantHoldBtn.Text = "⚡ Hold Time = 0"; InstantHoldBtn.BackgroundColor3 = Color3.fromRGB(140, 90, 0); InstantHoldBtn.TextColor3 = Color3.fromRGB(255, 255, 255); InstantHoldBtn.Font = Enum.Font.SourceSansBold; InstantHoldBtn.TextSize = 8; Instance.new("UICorner", InstantHoldBtn).CornerRadius = UDim.new(0, 4)  
local FirePromptsBtn = Instance.new("TextButton", Page3); FirePromptsBtn.Size = UDim2.new(0.16, 0, 0, 24); FirePromptsBtn.Position = UDim2.new(0.18, 0, 0, 0); FirePromptsBtn.Text = "🔥 Fire Prompts"; FirePromptsBtn.BackgroundColor3 = Color3.fromRGB(160, 40, 40); FirePromptsBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FirePromptsBtn.Font = Enum.Font.SourceSansBold; FirePromptsBtn.TextSize = 8; Instance.new("UICorner", FirePromptsBtn).CornerRadius = UDim.new(0, 4)  
local AutoLoopBtn = Instance.new("TextButton", Page3); AutoLoopBtn.Size = UDim2.new(0.16, 0, 0, 24); AutoLoopBtn.Position = UDim2.new(0.36, 0, 0, 0); AutoLoopBtn.Text = "🔄 Loop: OFF"; AutoLoopBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); AutoLoopBtn.TextColor3 = Color3.fromRGB(255, 255, 255); AutoLoopBtn.Font = Enum.Font.SourceSansBold; AutoLoopBtn.TextSize = 8; Instance.new("UICorner", AutoLoopBtn).CornerRadius = UDim.new(0, 4)  
local BlockEspToggle = Instance.new("TextButton", Page3); BlockEspToggle.Size = UDim2.new(0.16, 0, 0, 24); BlockEspToggle.Position = UDim2.new(0.54, 0, 0, 0); BlockEspToggle.Text = "🟩 ESP: OFF"; BlockEspToggle.BackgroundColor3 = Color3.fromRGB(50, 50, 50); BlockEspToggle.TextColor3 = Color3.fromRGB(255, 255, 255); BlockEspToggle.Font = Enum.Font.SourceSansBold; BlockEspToggle.TextSize = 8; Instance.new("UICorner", BlockEspToggle).CornerRadius = UDim.new(0, 4)  

-- Bộ lọc: thêm các nút lọc  
local FilterAllBtn = Instance.new("TextButton", Page3); FilterAllBtn.Size = UDim2.new(0.12, 0, 0, 24); FilterAllBtn.Position = UDim2.new(0.72, 0, 0, 0); FilterAllBtn.Text = "All"; FilterAllBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); FilterAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterAllBtn.Font = Enum.Font.SourceSansBold; FilterAllBtn.TextSize = 8; Instance.new("UICorner", FilterAllBtn).CornerRadius = UDim.new(0, 4)  
local FilterCollideBtn = Instance.new("TextButton", Page3); FilterCollideBtn.Size = UDim2.new(0.12, 0, 0, 24); FilterCollideBtn.Position = UDim2.new(0.85, 0, 0, 0); FilterCollideBtn.Text = "Vật lí"; FilterCollideBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterCollideBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterCollideBtn.Font = Enum.Font.SourceSansBold; FilterCollideBtn.TextSize = 8; Instance.new("UICorner", FilterCollideBtn).CornerRadius = UDim.new(0, 4)  
local FilterTransparentBtn = Instance.new("TextButton", Page3); FilterTransparentBtn.Size = UDim2.new(0.12, 0, 0, 24); FilterTransparentBtn.Position = UDim2.new(0, 0, 0, 28); FilterTransparentBtn.Text = "Trong suốt"; FilterTransparentBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterTransparentBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterTransparentBtn.Font = Enum.Font.SourceSansBold; FilterTransparentBtn.TextSize = 8; Instance.new("UICorner", FilterTransparentBtn).CornerRadius = UDim.new(0, 4)  
local FilterInvisibleBtn = Instance.new("TextButton", Page3); FilterInvisibleBtn.Size = UDim2.new(0.12, 0, 0, 24); FilterInvisibleBtn.Position = UDim2.new(0.14, 0, 0, 28); FilterInvisibleBtn.Text = "Tàng hình"; FilterInvisibleBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterInvisibleBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterInvisibleBtn.Font = Enum.Font.SourceSansBold; FilterInvisibleBtn.TextSize = 8; Instance.new("UICorner", FilterInvisibleBtn).CornerRadius = UDim.new(0, 4)  
local FilterTouchBtn = Instance.new("TextButton", Page3); FilterTouchBtn.Size = UDim2.new(0.12, 0, 0, 24); FilterTouchBtn.Position = UDim2.new(0.28, 0, 0, 28); FilterTouchBtn.Text = "Touch"; FilterTouchBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterTouchBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterTouchBtn.Font = Enum.Font.SourceSansBold; FilterTouchBtn.TextSize = 8; Instance.new("UICorner", FilterTouchBtn).CornerRadius = UDim.new(0, 4)  

local autoLoopActive = false  
local blockEspActive = false  
local blockHighlights = {}  
local activeBlockFilter = "All" -- All, Collide, Transparent, Invisible, Touch  

local ScanScroll = Instance.new("ScrollingFrame", Page3); ScanScroll.Size = UDim2.new(1, 0, 1, -56); ScanScroll.Position = UDim2.new(0, 0, 0, 56); ScanScroll.BackgroundTransparency = 1; ScanScroll.CanvasSize = UDim2.new(0, 0, 0, 0); ScanScroll.ScrollBarThickness = 3  
...
BlockEspToggle.MouseButton1Click:Connect(function()  
	...
	BlockEspToggle.Text = blockEspActive and "🟩 ESP: ON" or "🟩 ESP: OFF"  
...
-- Hàm filter  
local function setBlockFilter(filter)  
	activeBlockFilter = filter  
	FilterAllBtn.BackgroundColor3 = (filter == "All") and Color3.fromRGB(0, 140, 70) or Color3.fromRGB(50, 50, 50)  
	FilterCollideBtn.BackgroundColor3 = (filter == "Collide") and Color3.fromRGB(0, 140, 70) or Color3.fromRGB(50, 50, 50)  
	FilterTransparentBtn.BackgroundColor3 = (filter == "Transparent") and Color3.fromRGB(0, 140, 70) or Color3.fromRGB(50, 50, 50)  
	FilterInvisibleBtn.BackgroundColor3 = (filter == "Invisible") and Color3.fromRGB(0, 140, 70) or Color3.fromRGB(50, 50, 50)  
	FilterTouchBtn.BackgroundColor3 = (filter == "Touch") and Color3.fromRGB(0, 140, 70) or Color3.fromRGB(50, 50, 50)  
	updateBlockScanner()  
end  

FilterAllBtn.MouseButton1Click:Connect(function() setBlockFilter("All") end)  
FilterCollideBtn.MouseButton1Click:Connect(function() setBlockFilter("Collide") end)  
FilterTransparentBtn.MouseButton1Click:Connect(function() setBlockFilter("Transparent") end)  
FilterInvisibleBtn.MouseButton1Click:Connect(function() setBlockFilter("Invisible") end)  
FilterTouchBtn.MouseButton1Click:Connect(function() setBlockFilter("Touch") end)  
...
local function updateBlockScanner()  
	...
	for _, obj in pairs(Workspace:GetDescendants()) do  
		if obj:IsA("BasePart") then  
			...
			if activeBlockFilter == "All" then shouldShow = true  
			elseif activeBlockFilter == "Collide" and obj.CanCollide then shouldShow = true  
			elseif activeBlockFilter == "Transparent" and obj.Transparency > 0.5 and obj.Transparency < 1 then shouldShow = true  
			elseif activeBlockFilter == "Invisible" and obj.Transparency >= 1 then shouldShow = true  
			elseif activeBlockFilter == "Touch" and obj:FindFirstChildOfClass("TouchTransmitter") then shouldShow = true  
			end  
...
--=========================================================  
-- TAB 4: BACKPACK + QUÉT REPLICATEDSTORAGE & SERVERSERVICE + COPY ITEM (nâng cấp)  
--=========================================================  
local EquipAllBtn = Instance.new("TextButton", Page4); EquipAllBtn.Size = UDim2.new(0.24, 0, 0, 22); EquipAllBtn.Text = "⚔️ Equip All Tools"; EquipAllBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); EquipAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255); EquipAllBtn.Font = Enum.Font.SourceSansBold; EquipAllBtn.TextSize = 8; Instance.new("UICorner", EquipAllBtn).CornerRadius = UDim.new(0, 4)  
local DropAllBtn = Instance.new("TextButton", Page4); DropAllBtn.Size = UDim2.new(0.24, 0, 0, 22); DropAllBtn.Position = UDim2.new(0.26, 0, 0, 0); DropAllBtn.Text = "🗑️ Drop Holding Tool"; DropAllBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50); DropAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255); DropAllBtn.Font = Enum.Font.SourceSansBold; DropAllBtn.TextSize = 8; Instance.new("UICorner", DropAllBtn).CornerRadius = UDim.new(0, 4)  
local ScanStorageBtn = Instance.new("TextButton", Page4); ScanStorageBtn.Size = UDim2.new(0.24, 0, 0, 22); ScanStorageBtn.Position = UDim2.new(0.52, 0, 0, 0); ScanStorageBtn.Text = "📦 Scan Storage"; ScanStorageBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); ScanStorageBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ScanStorageBtn.Font = Enum.Font.SourceSansBold; ScanStorageBtn.TextSize = 8; Instance.new("UICorner", ScanStorageBtn).CornerRadius = UDim.new(0, 4)  
local CopyFromPlayerBtn = Instance.new("TextButton", Page4); CopyFromPlayerBtn.Size = UDim2.new(0.24, 0, 0, 22); CopyFromPlayerBtn.Position = UDim2.new(0.78, 0, 0, 0); CopyFromPlayerBtn.Text = "👥 Copy từ Player"; CopyFromPlayerBtn.BackgroundColor3 = Color3.fromRGB(0, 110, 170); CopyFromPlayerBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyFromPlayerBtn.Font = Enum.Font.SourceSansBold; CopyFromPlayerBtn.TextSize = 8; Instance.new("UICorner", CopyFromPlayerBtn).CornerRadius = UDim.new(0, 4)  
...
-- Hàm quét ReplicatedStorage và ServerScriptService  
local function scanStorage()  
	...
	Instance.new("UICorner", popup).CornerRadius = UDim.new(0, 8)  
	local close = Instance.new("TextButton", popup); close.Size = UDim2.new(0, 30, 0, 30); close.Position = UDim2.new(1, -35, 0, 5); close.Text = "✕"; close.BackgroundColor3 = Color3.fromRGB(50, 50, 50); close.TextColor3 = Color3.fromRGB(255, 255, 255); close.Font = Enum.Font.SourceSansBold; close.TextSize = 12; close.MouseButton1Click:Connect(function() popup:Destroy() end)  
	local title = Instance.new("TextLabel", popup); title.Size = UDim2.new(0.8, 0, 0, 30); title.Position = UDim2.new(0.05, 0, 0, 0); title.Text = "Items trong Storage"; title.TextColor3 = Color3.fromRGB(0, 255, 200); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold; title.TextSize = 12  
	local scroll = Instance.new("ScrollingFrame", popup); scroll.Size = UDim2.new(0.95, 0, 0.85, 0); scroll.Position = UDim2.new(0.025, 0, 0.1, 0); scroll.BackgroundColor3 = Color3.fromRGB(10, 10, 15); scroll.CanvasSize = UDim2.new(0, 0, 0, 0); scroll.ScrollBarThickness = 3  
	local listUI = Instance.new("UIListLayout", scroll); listUI.Padding = UDim.new(0, 4)  

	local items = {}  
	for _, container in pairs({ReplicatedStorage, ServerScriptService}) do  
		for _, child in pairs(container:GetChildren()) do  
			table.insert(items, {obj = child, parent = container.Name})  
		end  
	end  
	for _, item in ipairs(items) do  
		local frame = Instance.new("Frame", scroll); frame.Size = UDim2.new(1, -5, 0, 28); frame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 4)  
		local label = Instance.new("TextLabel", frame); label.Size = UDim2.new(0.4, 0, 1, 0); label.Position = UDim2.new(0.02, 0, 0, 0); label.Text = string.format("%s (%s)", item.obj.Name, item.parent); label.TextColor3 = Color3.fromRGB(200, 200, 200); label.BackgroundTransparency = 1; label.Font = Enum.Font.SourceSans; label.TextSize = 9; label.TextXAlignment = Enum.TextXAlignment.Left  
		local copyBtn = Instance.new("TextButton", frame); copyBtn.Size = UDim2.new(0.2, 0, 0.7, 0); copyBtn.Position = UDim2.new(0.42, 0, 0.15, 0); copyBtn.Text = "📋 Copy Path"; copyBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); copyBtn.TextColor3 = Color3.fromRGB(255, 255, 255); copyBtn.Font = Enum.Font.SourceSansBold; copyBtn.TextSize = 8; Instance.new("UICorner", copyBtn).CornerRadius = UDim.new(0, 3)  
		copyBtn.MouseButton1Click:Connect(function() if setclipboard then setclipboard(item.obj:GetFullName()) end end)  
		if item.obj:IsA("Tool") then  
			local cloneBtn = Instance.new("TextButton", frame); cloneBtn.Size = UDim2.new(0.2, 0, 0.7, 0); cloneBtn.Position = UDim2.new(0.64, 0, 0.15, 0); cloneBtn.Text = "📥 Clone to me"; cloneBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); cloneBtn.TextColor3 = Color3.fromRGB(255, 255, 255); cloneBtn.Font = Enum.Font.SourceSansBold; cloneBtn.TextSize = 8; Instance.new("UICorner", cloneBtn).CornerRadius = UDim.new(0, 3)  
			cloneBtn.MouseButton1Click:Connect(function()  
				pcall(function()  
					local clone = item.obj:Clone()  
...
ScanStorageBtn.MouseButton1Click:Connect(scanStorage)  

-- Copy từ player khác  
CopyFromPlayerBtn.MouseButton1Click:Connect(function()  
	local playerList = {}  
	for _, p in pairs(Players:GetPlayers()) do  
		if p ~= LocalPlayer then table.insert(playerList, p) end  
	end  
	if #playerList == 0 then return end  
	-- Tạo popup chọn player  
	local popup = Instance.new("Frame", ScreenGui)  
	...
	Instance.new("UICorner", popup).CornerRadius = UDim.new(0, 8)  
	local close = Instance.new("TextButton", popup); close.Size = UDim2.new(0, 30, 0, 30); close.Position = UDim2.new(1, -35, 0, 5); close.Text = "✕"; close.BackgroundColor3 = Color3.fromRGB(50, 50, 50); close.TextColor3 = Color3.fromRGB(255, 255, 255); close.Font = Enum.Font.SourceSansBold; close.TextSize = 12; close.MouseButton1Click:Connect(function() popup:Destroy() end)  
	local title = Instance.new("TextLabel", popup); title.Size = UDim2.new(0.8, 0, 0, 30); title.Position = UDim2.new(0.05, 0, 0, 0); title.Text = "Chọn player để copy"; title.TextColor3 = Color3.fromRGB(0, 255, 200); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold; title.TextSize = 12  
	local scroll = Instance.new("ScrollingFrame", popup); scroll.Size = UDim2.new(0.95, 0, 0.8, 0); scroll.Position = UDim2.new(0.025, 0, 0.15, 0); scroll.BackgroundColor3 = Color3.fromRGB(10, 10, 15); scroll.CanvasSize = UDim2.new(0, 0, 0, 0); scroll.ScrollBarThickness = 3  
	local listUI = Instance.new("UIListLayout", scroll); listUI.Padding = UDim.new(0, 4)  
	for _, p in ipairs(playerList) do  
		local btn = Instance.new("TextButton", scroll); btn.Size = UDim2.new(1, -5, 0, 28); btn.Text = p.Name; btn.BackgroundColor3 = Color3.fromRGB(30, 30, 35); btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.Font = Enum.Font.SourceSansBold; btn.TextSize = 12; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)  
		btn.MouseButton1Click:Connect(function()  
			pcall(function()  
				-- Copy tất cả tool từ backpack và character của player đó  
				local bp = p:FindFirstChild("Backpack")  
				local char = p.Character  
				local tools = {}  
				if bp then for _, t in pairs(bp:GetChildren()) do if t:IsA("Tool") then table.insert(tools, t) end end end  
...
--=========================================================  
-- TAB 5: STATS INSPECTOR + TÌM SCRIPT TĂNG CHỈ SỐ + BỘ LỌC (nâng cấp: filter script)  
--=========================================================  
...
local function findScriptsModifyingStat(statName, statValue, filter)  
	local results = {}  
	local allScripts = {}  
	for _, obj in pairs(Workspace:GetDescendants()) do  
	...
	for _, scr in ipairs(allScripts) do  
		-- Áp dụng filter (tên script hoặc path)  
		if filter and filter ~= "" then  
			local name = scr.Name:lower()  
			local path = scr:GetFullName():lower()  
			if not (name:find(filter:lower()) or path:find(filter:lower())) then  
				continue  
			end  
		end  
...
local function buildStatDetails(plr, parentFrame)  
	...
	for _, stat in pairs(leaderstats:GetChildren()) do  
		...
		local CopyValBtn = Instance.new("TextButton", StatFrame); CopyValBtn.Size = UDim2.new(0.15, 0, 0.7, 0); CopyValBtn.Position = UDim2.new(0.42, 0, 0.15, 0); CopyValBtn.Text = "📋 Value"; CopyValBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 50); CopyValBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyValBtn.Font = Enum.Font.SourceSansBold; CopyValBtn.TextSize = 8; Instance.new("UICorner", CopyValBtn).CornerRadius = UDim.new(0, 3)  
		CopyValBtn.MouseButton1Click:Connect(function() if setclipboard then setclipboard(valStr) end end)  
		-- Nút tìm script với ô nhập filter  
		local FindScriptBtn = Instance.new("TextButton", StatFrame); FindScriptBtn.Size = UDim2.new(0.2, 0, 0.7, 0); FindScriptBtn.Position = UDim2.new(0.58, 0, 0.15, 0); FindScriptBtn.Text = "🔍 Tìm script"; FindScriptBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); FindScriptBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FindScriptBtn.Font = Enum.Font.SourceSansBold; FindScriptBtn.TextSize = 8; Instance.new("UICorner", FindScriptBtn).CornerRadius = UDim.new(0, 3)  
		FindScriptBtn.MouseButton1Click:Connect(function()  
			-- Tạo popup với ô filter  
			local filterPopup = Instance.new("Frame", ScreenGui)  
			filterPopup.Size = UDim2.new(0, 400, 0, 100)  
			filterPopup.Position = UDim2.new(0.5, -200, 0.5, -50)  
			filterPopup.BackgroundColor3 = Color3.fromRGB(20, 20, 30)  
			Instance.new("UICorner", filterPopup).CornerRadius = UDim.new(0, 8)  
			local close = Instance.new("TextButton", filterPopup); close.Size = UDim2.new(0, 30, 0, 30); close.Position = UDim2.new(1, -35, 0, 5); close.Text = "✕"; close.BackgroundColor3 = Color3.fromRGB(50, 50, 50); close.TextColor3 = Color3.fromRGB(255, 255, 255); close.Font = Enum.Font.SourceSansBold; close.TextSize = 12; close.MouseButton1Click:Connect(function() filterPopup:Destroy() end)  
			local label = Instance.new("TextLabel", filterPopup); label.Size = UDim2.new(0.9, 0, 0, 25); label.Position = UDim2.new(0.05, 0, 0, 5); label.Text = "Nhập filter (tên script hoặc path):"; label.TextColor3 = Color3.fromRGB(200, 200, 200); label.BackgroundTransparency = 1; label.Font = Enum.Font.SourceSans; label.TextSize = 10; label.TextXAlignment = Enum.TextXAlignment.Left  
			local input = Instance.new("TextBox", filterPopup); input.Size = UDim2.new(0.6, 0, 0, 25); input.Position = UDim2.new(0.05, 0, 0.4, 0); input.PlaceholderText = "Filter..."; input.Text = ""; input.BackgroundColor3 = Color3.fromRGB(30, 30, 35); input.TextColor3 = Color3.fromRGB(255, 255, 255); input.Font = Enum.Font.SourceSans; input.TextSize = 10; Instance.new("UICorner", input).CornerRadius = UDim.new(0, 4)  
			local okBtn = Instance.new("TextButton", filterPopup); okBtn.Size = UDim2.new(0.2, 0, 0, 25); okBtn.Position = UDim2.new(0.7, 0, 0.4, 0); okBtn.Text = "OK"; okBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); okBtn.TextColor3 = Color3.fromRGB(255, 255, 255); okBtn.Font = Enum.Font.SourceSansBold; okBtn.TextSize = 10; Instance.new("UICorner", okBtn).CornerRadius = UDim.new(0, 4)  
			okBtn.MouseButton1Click:Connect(function()  
				local filterText = input.Text  
				local scripts = findScriptsModifyingStat(stat.Name, valStr, filterText)  
				filterPopup:Destroy()  
				-- Hiển thị kết quả  
				local popup = Instance.new("Frame", ScreenGui)  
				...
				popup.BackgroundColor3 = Color3.fromRGB(20, 20, 30)  
				Instance.new("UICorner", popup).CornerRadius = UDim.new(0, 8)  
				local close2 = Instance.new("TextButton", popup); close2.Size = UDim2.new(0, 30, 0, 30); close2.Position = UDim2.new(1, -35, 0, 5); close2.Text = "✕"; close2.BackgroundColor3 = Color3.fromRGB(50, 50, 50); close2.TextColor3 = Color3.fromRGB(255, 255, 255); close2.Font = Enum.Font.SourceSansBold; close2.TextSize = 12; close2.MouseButton1Click:Connect(function() popup:Destroy() end)  
				...
				local listUI = Instance.new("UIListLayout", scroll); listUI.Padding = UDim.new(0, 4)  
				if #scripts == 0 then  
					local empty = Instance.new("TextLabel", scroll); empty.Size = UDim2.new(1, 0, 0, 30); empty.Text = "Không tìm thấy script nào."; empty.BackgroundTransparency = 1; empty.TextColor3 = Color3.fromRGB(200, 200, 200); empty.Font = Enum.Font.SourceSans; empty.TextSize = 12  
				else  
					for _, scr in ipairs(scripts) do  
...
--=========================================================  
-- TAB 6: REMOTE SPY PRO (NÂNG CẤP TỐI ĐA)  
--=========================================================  
local SpyFilterBox = Instance.new("TextBox", Page6); SpyFilterBox.Size = UDim2.new(0.25, 0, 0, 24); SpyFilterBox.PlaceholderText = "🔍 Lọc Remote..."; SpyFilterBox.Text = ""; SpyFilterBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28); SpyFilterBox.TextColor3 = Color3.fromRGB(255, 255, 255); SpyFilterBox.Font = Enum.Font.SourceSans; SpyFilterBox.TextSize = 9; Instance.new("UICorner", SpyFilterBox).CornerRadius = UDim.new(0, 4)  
local RemoteSpyToggle = Instance.new("TextButton", Page6); RemoteSpyToggle.Size = UDim2.new(0.16, 0, 0, 24); RemoteSpyToggle.Position = UDim2.new(0.27, 0, 0, 0); RemoteSpyToggle.Text = "📡 Spy: OFF"; RemoteSpyToggle.BackgroundColor3 = Color3.fromRGB(140, 35, 35); RemoteSpyToggle.TextColor3 = Color3.fromRGB(255, 255, 255); RemoteSpyToggle.Font = Enum.Font.SourceSansBold; RemoteSpyToggle.TextSize = 9; Instance.new("UICorner", RemoteSpyToggle).CornerRadius = UDim.new(0, 4)  
local ScanRemotesBtn = Instance.new("TextButton", Page6); ScanRemotesBtn.Size = UDim2.new(0.16, 0, 0, 24); ScanRemotesBtn.Position = UDim2.new(0.45, 0, 0, 0); ScanRemotesBtn.Text = "🔍 Quét"; ScanRemotesBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); ScanRemotesBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ScanRemotesBtn.Font = Enum.Font.SourceSansBold; ScanRemotesBtn.TextSize = 8; Instance.new("UICorner", ScanRemotesBtn).CornerRadius = UDim.new(0, 4)  
local ClearSpyBtn = Instance.new("TextButton", Page6); ClearSpyBtn.Size = UDim2.new(0.16, 0, 0, 24); ClearSpyBtn.Position = UDim2.new(0.63, 0, 0, 0); ClearSpyBtn.Text = "🗑️ Clear Log"; ClearSpyBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60); ClearSpyBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ClearSpyBtn.Font = Enum.Font.SourceSansBold; ClearSpyBtn.TextSize = 9; Instance.new("UICorner", ClearSpyBtn).CornerRadius = UDim.new(0, 4)  
-- Nút block remote  
local BlockRemoteBtn = Instance.new("TextButton", Page6); BlockRemoteBtn.Size = UDim2.new(0.16, 0, 0, 24); BlockRemoteBtn.Position = UDim2.new(0.81, 0, 0, 0); BlockRemoteBtn.Text = "🚫 Block"; BlockRemoteBtn.BackgroundColor3 = Color3.fromRGB(180, 40, 40); BlockRemoteBtn.TextColor3 = Color3.fromRGB(255, 255, 255); BlockRemoteBtn.Font = Enum.Font.SourceSansBold; BlockRemoteBtn.TextSize = 8; Instance.new("UICorner", BlockRemoteBtn).CornerRadius = UDim.new(0, 4)  
...
BlockRemoteBtn.MouseButton1Click:Connect(function()  
	local rName = SpyFilterBox.Text:match("^(%S+)") or "default"  
	if rName then  
...
ScanRemotesBtn.MouseButton1Click:Connect(function()  
	...
	for _, r in ipairs(remotes) do  
		local frame = Instance.new("Frame", scroll); frame.Size = UDim2.new(1, -5, 0, 24); frame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 4)  
		local label = Instance.new("TextLabel", frame); label.Size = UDim2.new(0.4, 0, 1, 0); label.Position = UDim2.new(0.02, 0, 0, 0); label.Text = string.format("📡 %s (%s)", r.Name, r.ClassName); label.TextColor3 = Color3.fromRGB(200, 200, 200); label.BackgroundTransparency = 1; label.Font = Enum.Font.SourceSans; label.TextSize = 9; label.TextXAlignment = Enum.TextXAlignment.Left  
		local fireBtn = Instance.new("TextButton", frame); fireBtn.Size = UDim2.new(0.15, 0, 0.7, 0); fireBtn.Position = UDim2.new(0.42, 0, 0.15, 0); fireBtn.Text = "🔥 Fire"; fireBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); fireBtn.TextColor3 = Color3.fromRGB(255, 255, 255); fireBtn.Font = Enum.Font.SourceSansBold; fireBtn.TextSize = 8; Instance.new("UICorner", fireBtn).CornerRadius = UDim.new(0, 3)  
		...
		local copyBtn = Instance.new("TextButton", frame); copyBtn.Size = UDim2.new(0.2, 0, 0.7, 0); copyBtn.Position = UDim2.new(0.58, 0, 0.15, 0); copyBtn.Text = "📋 Copy Path"; copyBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); copyBtn.TextColor3 = Color3.fromRGB(255, 255, 255); copyBtn.Font = Enum.Font.SourceSansBold; copyBtn.TextSize = 8; Instance.new("UICorner", copyBtn).CornerRadius = UDim.new(0, 3)  
		copyBtn.MouseButton1Click:Connect(function() if setclipboard then setclipboard(r:GetFullName()) end end)  
		local blockBtn = Instance.new("TextButton", frame); blockBtn.Size = UDim2.new(0.2, 0, 0.7, 0); blockBtn.Position = UDim2.new(0.79, 0, 0.15, 0); blockBtn.Text = "🚫 Block"; blockBtn.BackgroundColor3 = Color3.fromRGB(180, 40, 40); blockBtn.TextColor3 = Color3.fromRGB(255, 255, 255); blockBtn.Font = Enum.Font.SourceSansBold; blockBtn.TextSize = 8; Instance.new("UICorner", blockBtn).CornerRadius = UDim.new(0, 3)  
		blockBtn.MouseButton1Click:Connect(function() blacklistedRemoteNames[r.Name] = true end)  
	end  
	scroll.CanvasSize = UDim2.new(0, 0, 0, listUI.AbsoluteContentSize.Y)  
end)  
...
local function logRemoteEvent(remote, args)  
		...
		if infoLabel then  
			...
			if args and #args > 0 then  
				...
				for i, v in ipairs(args) do  
					table.insert(argParts, tostring(v):sub(1, 20))  
				end  
				argStr = " | Args: " .. table.concat(argParts, ", ")  
			end  
			infoLabel.Text = string.format("⚡ [%s] %s (x%d)%s", remote.ClassName, rName, remoteCallCounts[remote], argStr)  
		end  
		return  
	end  

	local ItemFrame = Instance.new("Frame", SpyScroll); ItemFrame.Size = UDim2.new(1, -5, 0, 28); ItemFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 25); Instance.new("UICorner", ItemFrame).CornerRadius = UDim.new(0, 4)  
	local InfoText = Instance.new("TextLabel", ItemFrame); InfoText.Name = "InfoText"; InfoText.Size = UDim2.new(0.4, 0, 1, 0); InfoText.Position = UDim2.new(0.02, 0, 0, 0);  
	local argStr = ""  
	if args and #args > 0 then  
		local argParts = {}  
		for i, v in ipairs(args) do  
	...
	InfoText.Text = string.format("⚡ [%s] %s (x1)%s", remote.ClassName, rName, argStr)  
	InfoText.TextColor3 = Color3.fromRGB(255, 180, 50); InfoText.BackgroundTransparency = 1; InfoText.Font = Enum.Font.SourceSansBold; InfoText.TextSize = 9; InfoText.TextXAlignment = Enum.TextXAlignment.Left  
	local FireTestBtn = Instance.new("TextButton", ItemFrame); FireTestBtn.Size = UDim2.new(0.14, 0, 0.7, 0); FireTestBtn.Position = UDim2.new(0.44, 0, 0.15, 0); FireTestBtn.Text = "🔥 Fire"; FireTestBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); FireTestBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FireTestBtn.Font = Enum.Font.SourceSansBold; FireTestBtn.TextSize = 8; Instance.new("UICorner", FireTestBtn).CornerRadius = UDim.new(0, 3)  
	local CopyCallBtn = Instance.new("TextButton", ItemFrame); CopyCallBtn.Size = UDim2.new(0.14, 0, 0.7, 0); CopyCallBtn.Position = UDim2.new(0.59, 0, 0.15, 0); CopyCallBtn.Text = "📋 Copy Code"; CopyCallBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); CopyCallBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyCallBtn.Font = Enum.Font.SourceSansBold; CopyCallBtn.TextSize = 8; Instance.new("UICorner", CopyCallBtn).CornerRadius = UDim.new(0, 3)  
	local ViewArgsBtn = Instance.new("TextButton", ItemFrame); ViewArgsBtn.Size = UDim2.new(0.14, 0, 0.7, 0); ViewArgsBtn.Position = UDim2.new(0.74, 0, 0.15, 0); ViewArgsBtn.Text = "🔍 Args"; ViewArgsBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60); ViewArgsBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ViewArgsBtn.Font = Enum.Font.SourceSansBold; ViewArgsBtn.TextSize = 8; Instance.new("UICorner", ViewArgsBtn).CornerRadius = UDim.new(0, 3)  
	local PurgeBtn = Instance.new("TextButton", ItemFrame); PurgeBtn.Size = UDim2.new(0.14, 0, 0.7, 0); PurgeBtn.Position = UDim2.new(0.88, 0, 0.15, 0); PurgeBtn.Text = "🚫 Purge"; PurgeBtn.BackgroundColor3 = Color3.fromRGB(180, 40, 40); PurgeBtn.TextColor3 = Color3.fromRGB(255, 255, 255); PurgeBtn.Font = Enum.Font.SourceSansBold; PurgeBtn.TextSize = 8; Instance.new("UICorner", PurgeBtn).CornerRadius = UDim.new(0, 3)  
	...
	ViewArgsBtn.MouseButton1Click:Connect(function()  
		...
		Instance.new("UICorner", popup).CornerRadius = UDim.new(0, 8)  
		local close = Instance.new("TextButton", popup); close.Size = UDim2.new(0, 30, 0, 30); close.Position = UDim2.new(1, -35, 0, 5); close.Text = "✕"; close.BackgroundColor3 = Color3.fromRGB(50, 50, 50); close.TextColor3 = Color3.fromRGB(255, 255, 255); close.Font = Enum.Font.SourceSansBold; close.TextSize = 12; close.MouseButton1Click:Connect(function() popup:Destroy() end)  
		local title = Instance.new("TextLabel", popup); title.Size = UDim2.new(0.8, 0, 0, 30); title.Position = UDim2.new(0.05, 0, 0, 0); title.Text = "Arguments"; title.TextColor3 = Color3.fromRGB(0, 255, 200); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold; title.TextSize = 12  
		local scroll = Instance.new("ScrollingFrame", popup); scroll.Size = UDim2.new(0.95, 0, 0.85, 0); scroll.Position = UDim2.new(0.025, 0, 0.1, 0); scroll.BackgroundColor3 = Color3.fromRGB(10, 10, 15); scroll.CanvasSize = UDim2.new(0, 0, 0, 0); scroll.ScrollBarThickness = 3  
		local listUI = Instance.new("UIListLayout", scroll); listUI.Padding = UDim.new(0, 4)  
		for i, arg in ipairs(lastArgs) do  
			local frame = Instance.new("Frame", scroll); frame.Size = UDim2.new(1, -5, 0, 22); frame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 4)  
			local label = Instance.new("TextLabel", frame); label.Size = UDim2.new(1, 0, 1, 0); label.Position = UDim2.new(0.02, 0, 0, 0); label.Text = string.format("[%d] %s (%s)", i, tostring(arg), type(arg)); label.TextColor3 = Color3.fromRGB(200, 200, 200); label.BackgroundTransparency = 1; label.Font = Enum.Font.SourceSans; label.TextSize = 9; label.TextXAlignment = Enum.TextXAlignment.Left  
		end  
		scroll.CanvasSize = UDim2.new(0, 0, 0, listUI.AbsoluteContentSize.Y)  
	end)  
...
--=========================================================  
-- TAB 7: CONSOLE LOG (GIỐNG F9) (nâng cấp: thêm execute, copy all)  
--=========================================================  
local ConsoleSearchBox = Instance.new("TextBox", Page7); ConsoleSearchBox.Size = UDim2.new(0.25, 0, 0, 22); ConsoleSearchBox.PlaceholderText = "🔍 Tìm log..."; ConsoleSearchBox.Text = ""; ConsoleSearchBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28); ConsoleSearchBox.TextColor3 = Color3.fromRGB(255, 255, 255); ConsoleSearchBox.Font = Enum.Font.SourceSans; ConsoleSearchBox.TextSize = 9; Instance.new("UICorner", ConsoleSearchBox).CornerRadius = UDim.new(0, 3)  
local FilterErrorBtn = Instance.new("TextButton", Page7); FilterErrorBtn.Size = UDim2.new(0.1, 0, 0, 22); FilterErrorBtn.Position = UDim2.new(0.27, 0, 0, 0); FilterErrorBtn.Text = "❌ Lỗi"; FilterErrorBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterErrorBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterErrorBtn.Font = Enum.Font.SourceSansBold; FilterErrorBtn.TextSize = 8; Instance.new("UICorner", FilterErrorBtn).CornerRadius = UDim.new(0, 3)  
local FilterWarnBtn = Instance.new("TextButton", Page7); FilterWarnBtn.Size = UDim2.new(0.1, 0, 0, 22); FilterWarnBtn.Position = UDim2.new(0.38, 0, 0, 0); FilterWarnBtn.Text = "⚠️ Cảnh báo"; FilterWarnBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterWarnBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterWarnBtn.Font = Enum.Font.SourceSansBold; FilterWarnBtn.TextSize = 8; Instance.new("UICorner", FilterWarnBtn).CornerRadius = UDim.new(0, 3)  
local FilterInfoBtn = Instance.new("TextButton", Page7); FilterInfoBtn.Size = UDim2.new(0.1, 0, 0, 22); FilterInfoBtn.Position = UDim2.new(0.49, 0, 0, 0); FilterInfoBtn.Text = "ℹ️ Thông tin"; FilterInfoBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50); FilterInfoBtn.TextColor3 = Color3.fromRGB(255, 255, 255); FilterInfoBtn.Font = Enum.Font.SourceSansBold; FilterInfoBtn.TextSize = 8; Instance.new("UICorner", FilterInfoBtn).CornerRadius = UDim.new(0, 3)  
local ClearConsoleBtn = Instance.new("TextButton", Page7); ClearConsoleBtn.Size = UDim2.new(0.1, 0, 0, 22); ClearConsoleBtn.Position = UDim2.new(0.60, 0, 0, 0); ClearConsoleBtn.Text = "🗑️ Clear"; ClearConsoleBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60); ClearConsoleBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ClearConsoleBtn.Font = Enum.Font.SourceSansBold; ClearConsoleBtn.TextSize = 8; Instance.new("UICorner", ClearConsoleBtn).CornerRadius = UDim.new(0, 3)  
local CopyAllBtn = Instance.new("TextButton", Page7); CopyAllBtn.Size = UDim2.new(0.12, 0, 0, 22); CopyAllBtn.Position = UDim2.new(0.72, 0, 0, 0); CopyAllBtn.Text = "📋 Copy All"; CopyAllBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); CopyAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyAllBtn.Font = Enum.Font.SourceSansBold; CopyAllBtn.TextSize = 8; Instance.new("UICorner", CopyAllBtn).CornerRadius = UDim.new(0, 3)  

local ConsoleScroll = Instance.new("ScrollingFrame", Page7); ConsoleScroll.Size = UDim2.new(1, 0, 1, -58); ConsoleScroll.Position = UDim2.new(0, 0, 0, 28); ConsoleScroll.BackgroundTransparency = 1; ConsoleScroll.CanvasSize = UDim2.new(0, 0, 0, 0); ConsoleScroll.ScrollBarThickness = 3  
local ConsoleListUI = Instance.new("UIListLayout", ConsoleScroll); ConsoleListUI.Padding = UDim.new(0, 3)  

-- Execute box  
local ExecuteBox = Instance.new("TextBox", Page7); ExecuteBox.Size = UDim2.new(0.7, 0, 0, 24); ExecuteBox.Position = UDim2.new(0, 0, 1, -24); ExecuteBox.PlaceholderText = "Nhập lệnh Lua..."; ExecuteBox.Text = ""; ExecuteBox.BackgroundColor3 = Color3.fromRGB(20, 20, 25); ExecuteBox.TextColor3 = Color3.fromRGB(255, 255, 255); ExecuteBox.Font = Enum.Font.SourceSans; ExecuteBox.TextSize = 10; Instance.new("UICorner", ExecuteBox).CornerRadius = UDim.new(0, 4)  
local ExecuteBtn = Instance.new("TextButton", Page7); ExecuteBtn.Size = UDim2.new(0.18, 0, 0, 24); ExecuteBtn.Position = UDim2.new(0.72, 0, 1, -24); ExecuteBtn.Text = "▶️ Execute"; ExecuteBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); ExecuteBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ExecuteBtn.Font = Enum.Font.SourceSansBold; ExecuteBtn.TextSize = 10; Instance.new("UICorner", ExecuteBtn).CornerRadius = UDim.new(0, 4)  
...
local function refreshConsoleLogs()  
	...
	for _, entry in ipairs(logHistory) do  
		...
		if show and (searchKeyword == "" or string.find(string.lower(entry.msg), searchKeyword)) then  
			...
			local LogText = Instance.new("TextLabel", Frame); LogText.Size = UDim2.new(0.98, 0, 1, 0); LogText.Position = UDim2.new(0.01, 0, 0, 0); LogText.Text = string.format("[%s] %s", entry.time or "??", entry.msg); LogText.BackgroundTransparency = 1; LogText.Font = Enum.Font.SourceSans; LogText.TextSize = 9; LogText.TextXAlignment = Enum.TextXAlignment.Left  
			if entry.type == Enum.MessageType.MessageOutput then LogText.TextColor3 = Color3.fromRGB(220, 220, 220)  
			elseif entry.type == Enum.MessageType.MessageWarning then LogText.TextColor3 = Color3.fromRGB(255, 200, 50)  
			elseif entry.type == Enum.MessageType.MessageError then LogText.TextColor3 = Color3.fromRGB(255, 70, 70)  
...
local function appendLog(message, messageType)  
	local time = os.date("%H:%M:%S")  
	table.insert(logHistory, {msg = message, type = messageType, time = time})  
	refreshConsoleLogs()  
end  
...
CopyAllBtn.MouseButton1Click:Connect(function()  
	local allText = ""  
	for _, entry in ipairs(logHistory) do  
		allText = allText .. string.format("[%s] %s\n", entry.time or "??", entry.msg)  
	end  
	if setclipboard then setclipboard(allText) end  
end)  

--=========================================================  
-- TAB 8: GUI INSPECTOR + ĂN GUI (nâng cấp: clone GUI)  
--=========================================================  
local GuiTargetBox = Instance.new("TextBox", Page8); GuiTargetBox.Size = UDim2.new(0, 240, 0, 24); GuiTargetBox.PlaceholderText = "🔍 Tên Player (Trống = Bản thân)..."; GuiTargetBox.Text = ""; GuiTargetBox.BackgroundColor3 = Color3.fromRGB(24, 24, 28); GuiTargetBox.TextColor3 = Color3.fromRGB(255, 255, 255); GuiTargetBox.Font = Enum.Font.SourceSans; GuiTargetBox.TextSize = 9; Instance.new("UICorner", GuiTargetBox).CornerRadius = UDim.new(0, 4)  
local ScanGuiBtn = Instance.new("TextButton", Page8); ScanGuiBtn.Size = UDim2.new(0, 100, 0, 24); ScanGuiBtn.Position = UDim2.new(0, 250, 0, 0); ScanGuiBtn.Text = "🔍 Quét GUI"; ScanGuiBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 70); ScanGuiBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ScanGuiBtn.Font = Enum.Font.SourceSansBold; ScanGuiBtn.TextSize = 8; Instance.new("UICorner", ScanGuiBtn).CornerRadius = UDim.new(0, 4)  
local ToggleAllGuiBtn = Instance.new("TextButton", Page8); ToggleAllGuiBtn.Size = UDim2.new(0, 100, 0, 24); ToggleAllGuiBtn.Position = UDim2.new(0, 360, 0, 0); ToggleAllGuiBtn.Text = "👁️ Bật/Tắt All"; ToggleAllGuiBtn.BackgroundColor3 = Color3.fromRGB(0, 110, 180); ToggleAllGuiBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ToggleAllGuiBtn.Font = Enum.Font.SourceSansBold; ToggleAllGuiBtn.TextSize = 8; Instance.new("UICorner", ToggleAllGuiBtn).CornerRadius = UDim.new(0, 4)  
local CopyGuiBtn = Instance.new("TextButton", Page8); CopyGuiBtn.Size = UDim2.new(0, 100, 0, 24); CopyGuiBtn.Position = UDim2.new(0, 470, 0, 0); CopyGuiBtn.Text = "📋 Copy GUI"; CopyGuiBtn.BackgroundColor3 = Color3.fromRGB(180, 90, 0); CopyGuiBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CopyGuiBtn.Font = Enum.Font.SourceSansBold; CopyGuiBtn.TextSize = 8; Instance.new("UICorner", CopyGuiBtn).CornerRadius = UDim.new(0, 4)  
...
local currentGuiList = {} -- lưu các ScreenGui được quét  
...
local function scanGuiOfPlayer(targetPlayer)  
	...
	for _, screenGui in pairs(pGui:GetChildren()) do  
		if screenGui:IsA("ScreenGui") then  
			table.insert(currentGuiList, screenGui)  
			local innerScripts = {}  
			...
			local InfoText = Instance.new("TextLabel", Frame); InfoText.Size = UDim2.new(0.5, 0, 0, 18); InfoText.Position = UDim2.new(0.02, 0, 0, 4); InfoText.Text = string.format("🖥️ %s (Enabled: %s)", screenGui.Name, tostring(screenGui.Enabled)); InfoText.TextColor3 = Color3.fromRGB(255, 210, 80); InfoText.BackgroundTransparency = 1; InfoText.Font = Enum.Font.SourceSansBold; InfoText.TextSize = 10; InfoText.TextXAlignment = Enum.TextXAlignment.Left  
			local ToggleVisBtn = Instance.new("TextButton", Frame); ToggleVisBtn.Size = UDim2.new(0.2, 0, 0, 20); ToggleVisBtn.Position = UDim2.new(0.52, 0, 0, 4); ToggleVisBtn.Text = screenGui.Enabled and "👁️ Bật/Tắt" or "🙈 Đang Ẩn"; ToggleVisBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60); ToggleVisBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ToggleVisBtn.Font = Enum.Font.SourceSansBold; ToggleVisBtn.TextSize = 8; Instance.new("UICorner", ToggleVisBtn).CornerRadius = UDim.new(0, 3)  
			ToggleVisBtn.MouseButton1Click:Connect(function() screenGui.Enabled = not screenGui.Enabled; ToggleVisBtn.Text = screenGui.Enabled and "👁️ Bật/Tắt" or "🙈 Đang Ẩn" end)  
			-- Nút copy GUI  
			local CloneGuiBtn = Instance.new("TextButton", Frame); CloneGuiBtn.Size = UDim2.new(0.2, 0, 0, 20); CloneGuiBtn.Position = UDim2.new(0.74, 0, 0, 4); CloneGuiBtn.Text = "📋 Clone to me"; CloneGuiBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180); CloneGuiBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CloneGuiBtn.Font = Enum.Font.SourceSansBold; CloneGuiBtn.TextSize = 8; Instance.new("UICorner", CloneGuiBtn).CornerRadius = UDim.new(0, 3)  
			CloneGuiBtn.MouseButton1Click:Connect(function()  
				pcall(function()  
...
-- Copy GUI (lấy GUI của player đang quét)  
CopyGuiBtn.MouseButton1Click:Connect(function()  
	pcall(function()  
		local pGui = currentScannedGuiPlayer:FindFirstChild("PlayerGui")  
		if pGui then  
			for _, sg in pairs(pGui:GetChildren()) do  
...
--=========================================================  
-- TAB 9: HITBOX MASTER (nâng cấp: thêm điều chỉnh màu sắc và kích thước toàn cục)  
--=========================================================  
...
local function createHitboxCard(title, configKey, hasExpand)  
	...
	Card.Size = UDim2.new(1, -5, 0, hasExpand and 90 or 65)  
	...
	BoxToggleBtn.Size = UDim2.new(0.22, 0, 0, 22)  
	...
	XRayToggleBtn.Size = UDim2.new(0.22, 0, 0, 22)  
	XRayToggleBtn.Position = UDim2.new(0.59, 0, 0, 4)  
	...
	-- Color picker (dùng TextBox nhập hex)  
	local ColorInput = Instance.new("TextBox", Card)  
	ColorInput.Size = UDim2.new(0.16, 0, 0, 22)  
	ColorInput.Position = UDim2.new(0.82, 0, 0, 4)  
	ColorInput.PlaceholderText = "Hex color"  
	ColorInput.Text = "#FF0000"  
	ColorInput.BackgroundColor3 = Color3.fromRGB(15, 15, 18)  
	ColorInput.TextColor3 = Color3.fromRGB(255, 255, 255)  
	ColorInput.Font = Enum.Font.SourceSans  
	ColorInput.TextSize = 8  
	Instance.new("UICorner", ColorInput).CornerRadius = UDim.new(0, 3)  
	...
	ColorInput.FocusLost:Connect(function()  
		pcall(function()  
			local hex = ColorInput.Text:gsub("#", "")  
			local r = tonumber(hex:sub(1,2), 16) or 255  
			local g = tonumber(hex:sub(3,4), 16) or 0  
			local b = tonumber(hex:sub(5,6), 16) or 0  
			cfg.Color = Color3.fromRGB(r, g, b)  
			applyHitboxVisuals()  
		end)  
	end)  
	...
	if hasExpand then  
		...
		ExpandToggleBtn.Size = UDim2.new(0.3, 0, 0, 22)  
		ExpandToggleBtn.Position = UDim2.new(0.02, 0, 0, 34)  
		ExpandToggleBtn.Text = "💥 Expand: OFF"  
		...
		SizeInput.Size = UDim2.new(0.3, 0, 0, 22)  
		SizeInput.Position = UDim2.new(0.34, 0, 0, 34)  
		...
		ExpandToggleBtn.MouseButton1Click:Connect(function()  
			...
			ExpandToggleBtn.Text = cfg.Expand and "💥 Expand: ON" or "💥 Expand: OFF"  
...
createHitboxCard("🧱 Khối Thường", "Blocks", false)  
createHitboxCard("👻 Khối Tàng Hình", "InvisBlocks", false)  
createHitboxCard("⚡ Touch/Trigger", "TouchTriggers", false)  

HitboxScroll.CanvasSize = UDim2.new(0, 0, 0, HitboxListUI.AbsoluteContentSize.Y)  

-- Nút Apply All Changes (để cập nhật toàn bộ)  
local ApplyAllBtn = Instance.new("TextButton", Page9)  
ApplyAllBtn.Size = UDim2.new(1, 0, 0, 24)  
ApplyAllBtn.Position = UDim2.new(0, 0, 1, -24)  
ApplyAllBtn.Text = "🔄 Apply All Changes"  
ApplyAllBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 80)  
ApplyAllBtn.TextColor3 = Color3.fromRGB(255, 255, 255)  
ApplyAllBtn.Font = Enum.Font.SourceSansBold  
ApplyAllBtn.TextSize = 10  
Instance.new("UICorner", ApplyAllBtn).CornerRadius = UDim.new(0, 4)  
ApplyAllBtn.MouseButton1Click:Connect(applyHitboxVisuals)  
